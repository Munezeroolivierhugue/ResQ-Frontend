  ...
  useEffect(() => {
    listDataQuality().then((records) => {
      const mapped = records.map((r) => ({
        ...
        status: r.overall_score >= 0.9 ? 'OK' : r.overall_score >= 0.7 ? 'DEGRADED' : 'ERROR',
        completeness_pct: Math.round((r.completeness ?? 0) * 100),
        last_updated_at: r.checked_at ? new Date(r.checked_at).toLocaleTimeString() : '—',
        border: r.overall_score >= 0.9 ? 'var(--status-low)' : r.overall_score >= 0.7 ? 'var(--status-medium)' : 'var(--status-critical)'
      }))
