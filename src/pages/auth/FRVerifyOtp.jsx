...
import { Siren, Lock, RefreshCw } from 'lucide-react'
import FRAuthShell from '../../components/auth/FRAuthShell'

const OTP_LEN = 6

export default function FRVerifyOtp() {
  const navigate = useNavigate()
  const [digits, setDigits] = useState(Array(OTP_LEN).fill(''))
  const [error, setError] = useState('')
  const refs = useRef([])

  const email = sessionStorage.getItem('resq-register-email') || 'responder@rnp.gov.rw'
    ...
    <FRAuthShell>
      <div className="fr-auth-card-wrap">
        ...
        <div className="fr-auth-card">
          ...
          <h2 className="fr-auth-card-title">Confirm Your Identity</h2>
