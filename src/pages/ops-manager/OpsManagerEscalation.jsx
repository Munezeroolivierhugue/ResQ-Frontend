...
import { getEscalationDetail, OPS_AGENCIES } from '../../data/mockOpsManagerData'
  ...
  return (
    <div className="portal-page portal-split-60-40 min-h-full relative">
      ...
      <DispatchUnitsModal
        ...
        onConfirm={(units) => setToast(`${units.length} unit${units.length > 1 ? 's' : ''} dispatched to ${detail.id}`)}
      />
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" className="bg-transparent border-none cursor-pointer text-(--text-muted)" onClick={() => setNotifyOpen(false)}>
                <X size={18} />
              </button>
            </div>
            <p className="text-[12px] text-(--text-secondary) m-0 mb-3">
              Incident: <span className="font-mono text-(--accent)">{detail.id}</span>
            </p>
            <div className="flex flex-col gap-2">
              {NON_RNP_AGENCIES.map((a) => (
                <label key={a.id} className="flex items-center gap-3 cursor-pointer p-2 rounded hover:bg-(--bg-elevated)">
                  <input
                    type="radio"
                    name="agency"
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-(--text-muted)">{a.units} units · {a.status}</div>
                  </div>
                </label>
              ))}
            </div>
            <div className="flex gap-2 mt-4">
              ...
              <button type="button" className="dispatcher-btn-ghost text-[12px]" onClick={() => { setNotifyOpen(false); setSelectedAgency(null) }}>
                Cancel
              </button>
            </div>
          </div>
      ...
      <div className="flex flex-col gap-4 min-w-0">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            style={{
              border: dcEscalated ? '1px solid var(--status-low)' : '1px solid var(--status-critical)',
              color: dcEscalated ? 'var(--status-low)' : 'var(--status-critical)',
              background: dcEscalated ? 'var(--status-low-bg)' : 'var(--status-critical-bg)',
            }}
            onClick={handleEscalateToBC}
            disabled={dcEscalated}
          >
            <ChevronsUp size={16} /> {dcEscalated ? 'Escalated to District Commander ✓' : 'Escalate to District Commander'}
          </button>
        </div>
        ...
        <div className="dispatcher-surface p-4">
          ...
          {detail.units.map((u) => (
            <div key={u.id} className="flex flex-wrap items-center gap-2 py-2 border-b border-(--border-subtle) text-[12px] last:border-0">
              ...
              <span className="text-(--text-secondary)">{u.eta_minutes != null ? `${u.eta_minutes} min` : 'On scene'}</span>
