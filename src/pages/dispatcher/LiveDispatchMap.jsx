...
export default function LiveDispatchMap() {
  ...
  const [unitFilter, setUnitFilter] = useState('Available')
  ...
  // Unique districts from loaded vehicles for the district selector
  const districtOptions = [
    { value: 'All', label: 'All Districts', icon: MapPin },
    ...Array.from(new Set(vehicles.map(v => v.district_name).filter(Boolean)))
      .sort()
      .map(d => ({ value: d, label: d, icon: MapPin })),
  ]

  const filteredUnits = vehicles.filter(u => {
    const statusMatch = unitFilter === 'All' || u.status === unitFilter.toLowerCase()
    const districtMatch = districtFilter === 'All' || u.district_name === districtFilter
