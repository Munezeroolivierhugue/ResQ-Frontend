...
export default function SettingsProfileSection({
  ...
  const [savedForm, setSavedForm] = useState({ name: '', email: '', phone: '' })
  ...
  useEffect(() => {
    if (!userId || userId === 'demo-user-uuid') {
      const f = { name: sessionUser?.full_name || '', email: sessionUser?.email || '', phone: '' }
      setForm(f)
      setSavedForm(f)
    ...
    getUser(userId)
      .then((u) => {
        setUser(u)
        const f = { name: u.full_name || '', email: u.email || '', phone: u.phone_number || '' }
        setForm(f)
        setSavedForm(f)
        onUserLoaded?.(u)
      })
  ...
  const handleSave = async () => {
    ...
    try {
      if (userId && userId !== 'demo-user-uuid') {
        const updated = await updateUser(userId, { full_name: form.name, phone_number: form.phone })
        const f = { name: updated.full_name || form.name, email: updated.email || form.email, phone: updated.phone_number || form.phone }
        setForm(f)
        setSavedForm(f)
      } else {
        setSavedForm({ ...form })
  ...
  const roleLabel = ROLE_LABELS[user?.role] || ROLE_LABELS[sessionUser?.role?.toUpperCase()] || 'USER'
  ...
  return (
    <div className="settings-profile-section flex flex-col gap-4 relative">
      <div className="dispatcher-surface flex flex-wrap items-center gap-4 w-full" style={{ padding: '1.25rem 1.5rem' }}>
        <span
        ...
        <div className="flex-1 min-w-[200px]">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              ...
              style={{ background: 'var(--accent-ghost)', color: 'var(--accent)', borderColor: 'var(--accent)', fontFamily: 'var(--font-display)' }}
            >
              {roleLabel}
            </span>
          </div>
      ...
      <div className="settings-section-card dispatcher-surface p-5 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {shiftStats.map((stat) => (
            ...
            >
              <div className="text-[10px] uppercase tracking-wider text-(--text-muted) mb-1" style={{ fontFamily: 'var(--font-mono)' }}>
                {stat.label}
              </div>
              <div className={`text-[16px] font-semibold text-(--text-primary)${stat.mono ? ' font-mono' : ''}`}>
                {stat.value}
              </div>
            </div>
          ))}
      ...
      {isEditing && (
        <div className="settings-profile-actions sticky bottom-0 z-10 flex justify-end gap-3 py-3 border-t border-(--border-subtle) bg-(--bg-surface)">
          <button type="button" className="dispatcher-btn-ghost text-[13px]" onClick={handleDiscard} disabled={saving}>
            Discard Changes
          </button>
          <button
            type="button"
...
function DutyStatusPill({ duty }) {
  if (duty === 'off_duty') {
    return (
      <span className="text-[11px] font-semibold px-2.5 py-1 rounded border" style={{ background: 'var(--bg-elevated)', color: 'var(--text-muted)', borderColor: 'var(--border)' }}>
        ● Off Duty
      </span>
    )
  }
  if (duty === 'on_break') {
    return (
      <span className="text-[11px] font-semibold px-2.5 py-1 rounded border" style={{ background: 'var(--status-medium-bg)', color: 'var(--status-medium)', borderColor: 'var(--status-medium)' }}>
        ● On Break
      </span>
    )
  }
  return (
    <span className="text-[11px] font-semibold px-2.5 py-1 rounded border inline-flex items-center gap-1" style={{ background: 'var(--status-low-bg)', color: 'var(--status-low)', borderColor: 'var(--status-low)' }}>
