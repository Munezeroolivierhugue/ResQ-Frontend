...
function ShiftStatTile({ label, value, mono }) {
  return (
    ...
    >
      ...
      <div className={`text-[16px] font-semibold text-(--text-primary)${mono ? ' font-mono' : ''}`}>
        {value}
      </div>
    </div>
  )
...
export default function SettingsShiftManagementSection({
  ...
  const isOps = variant === 'ops_manager'
  const [shiftStart, setShiftStart] = useState(isOps ? OM_DEFAULTS.shiftStart : DISPATCHER_DEFAULTS.shiftStart)
  const [shiftEnd, setShiftEnd] = useState(isOps ? OM_DEFAULTS.shiftEnd : DISPATCHER_DEFAULTS.shiftEnd)
  const [handoverReminder, setHandoverReminder] = useState(DISPATCHER_DEFAULTS.handoverReminder)
  const [incomingReminder, setIncomingReminder] = useState(OM_DEFAULTS.incomingReminder)
  const [outgoingReminder, setOutgoingReminder] = useState(OM_DEFAULTS.outgoingReminder)
  ...
  return (
    <div className="settings-section-card dispatcher-surface p-5 w-full">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-1">
        <div>
          <h2 className="text-base font-bold m-0" style={{ fontFamily: 'var(--font-display)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {isOps ? (
          <>
            <ShiftStatTile label="Shift window" value={`${shiftStart} – ${shiftEnd} · Today`} />
            <ShiftStatTile label="Time on command" value={formatElapsed(elapsed)} mono />
            <ShiftStatTile label="Escalations managed" value="4" />
            <ShiftStatTile label="DC report" value="Pending submit" />
          </>
        ) : (
          <>
            <ShiftStatTile label="Shift" value={`${shiftStart} – ${shiftEnd} · Today`} />
            <ShiftStatTile label="Incidents handled today" value="14" />
            <ShiftStatTile label="Time on duty" value={formatElapsed(elapsed)} mono />
            <ShiftStatTile label="Assigned district" value="Nyarugenge / Kicukiro" />
          </>
        )}
      </div>
        ...
        <>
          <label className="settings-form-field dispatcher-field mb-4 block">
            ...
            <select
              ...
              onChange={(e) => { setIncomingReminder(e.target.value); onSave?.() }}
            >
            ...
            <span className="text-[11px] text-(--text-muted) mt-1 block">
              Prompts you to read the briefing from the outgoing Operations Manager
            </span>
          </label>
          ...
          <label className="settings-form-field dispatcher-field mb-4 block">
            ...
            <select
              ...
              onChange={(e) => { setOutgoingReminder(e.target.value); onSave?.() }}
            >
          ...
          >
            ...
            <span className="text-(--text-secondary)">
              Incoming OM shift 16:00–00:00 · Outgoing briefing due by {shiftEnd}
            </span>
          </div>
        </>
      ) : (
        <>
          <label className="settings-form-field dispatcher-field mb-4 block">
            ...
            <select
              ...
              onChange={(e) => { setHandoverReminder(e.target.value); onSave?.() }}
            >
      ...
      <button type="button" className="dispatcher-btn-primary mt-4" onClick={handleSave}>
        Save changes
      </button>
    </div>
  )
