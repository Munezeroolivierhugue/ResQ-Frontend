    ...
    >
      ...
      {section === 'notifications' && (
        <div className="settings-section-card dispatcher-surface p-5 w-full">
          ...
          <p className="text-[13px] text-(--text-secondary) m-0 mb-4">Alerts for command oversight and escalations.</p>
          <SettingsGroup title="Command Alerts">
            <SettingsToggleRow label="Critical escalations" description="Immediate alert when incidents escalate to command" on={toggles.escalationCritical} onChange={(v) => setToggle('escalationCritical', v)} />
            <SettingsToggleRow label="AI reallocation recommendations" description="When new AI resource moves are pending approval" on={toggles.aiRecommendations} onChange={(v) => setToggle('aiRecommendations', v)} />
            <SettingsToggleRow label="Dispatcher overload warnings" description="When a dispatcher exceeds workload thresholds" on={toggles.dispatcherOverload} onChange={(v) => setToggle('dispatcherOverload', v)} />
          </SettingsGroup>
          <SettingsGroup title="Operations">
            <SettingsToggleRow label="Mutual aid request updates" description="Status changes on cross-district aid requests" on={toggles.mutualAid} onChange={(v) => setToggle('mutualAid', v)} />
            <SettingsToggleRow label="Shift handover briefings" description="Incoming handover documents from previous shift" on={toggles.shiftHandover} onChange={(v) => setToggle('shiftHandover', v)} />
      ...
      {section === 'shift' && (
        <SettingsShiftManagementSection variant="ops_manager" onSave={flashToast} />
      )}
